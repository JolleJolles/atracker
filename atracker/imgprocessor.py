...
class ImgProcessor:
    ...
    def __init__(self, img, img_bg, img_mask=None, treshtype="bw", blur=9,
        erode=1, blur2=1, treshold=50, min_area=100, max_area=10000, croppad=20,
    ...
    def process(self):
        ...
        if self.treshtype.startswith("bw"):
            ...
            self.img_tresh = cv2.GaussianBlur(self.img_tresh, (self.blur2, self.blur2), 3)
            self.img_tresh = cv2.threshold(self.img_tresh, self.treshold, 255, cv2.THRESH_BINARY)[1]
        else:
            self.img_tresh = cv2.GaussianBlur(self.img, (self.blur, self.blur), 0)
            self.img_tresh = cv2.cvtColor(self.img_tresh, cv2.COLOR_BGR2HSV)
            self.img_tresh = cv2.inRange(self.img_tresh, self.colmin, self.colmax)
            self.img_tresh = cv2.erode(self.img_tresh, None, iterations=2)
            self.img_tresh = cv2.dilate(self.img_tresh, None, iterations=2)

        # Extract contours
        allcons,_ = cv2.findContours(self.img_tresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[-2:]
        ...
        if self.min_area < area < self.max_area and min_aspect_ratio < aspect_ratio < max_aspect_ratio:
            ...
            if self.simple:
                ...
                if len(skelpts)>0:
                    ...
                    self.conlist["icom"][ind] = self.conlist["com"][ind]
