...
In case of major tracking issues, across all videos, it indicates that the treshold parameters were not optimal, potentially the background file not perfect, or alternatively there were some issues with the video, such as changing light levels. Based on the severity and type of issues determined, it could be a solution to try and improve the treshold parameters. In some cases the best solution is to create a new tresh_info file and only use that on the selected videos with issues.
...
AT.process(pools=1, names=None, overwrite=False, fulldata=True, convert=True, removeoutliers=True, 
           ...
           manonly=True, man_frameloc=0, man_timewindow=0, man_treshold_speed=0, smoothwin=10, 
           ...
           headtoorientspeedtresh=1, fillmissingorientdifftresh=100, centertype = "walls", 
           centralise=False)
```
...
12. Data column added with status if object is under/out of the mask and frames under and within treshold value from the mask are removed
...
The manual tracking function is a very handy interactive tool that works with a number of keypresses and listens to mouse movements, similar as the `set_interactive` function. It opens a video window, a scrollbar for changing the video frame, a scrollbar for changing the time window for which tracking data should be shown, an info panel with information about the coordinates in focus, a scrollbar to change the speed treshold, and a scrollbar to change the smoothing window.
...
For changing orientation data it may be helpful to differently plot the trajectory below a certain speed treshold, so it is clear what data needs to be deleted. For this you can use the speed treshold scrollbar. We can also set the smoothing window dynamically to find the window for which the data is optimally smoothed.
...
In addition, we can set a number of helpful parameters to run the manual tracker, including the frame to start with (`man_frameloc`), the time window to start with (`man_timewindow`), and the speed treshold (`man_treshold_speed`) below which data will be visualised differently.
...
- `mintrajlength`
- `filllentresh_com`
- `filllentresh_headtail`
- `headtoorientspeedtresh`
- `fillmissingorientdifftresh`
